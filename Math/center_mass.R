...
center_of_mass <- function(f, g, lower, upper){ #Center of mass
	area <- integ(f, lower, upper, 200) - integ(g, lower, upper, 200)
	...
	c_x <- 1/area * integ(I_x, lower, upper, 300)
	c_y <- 1/area * integ(I_y, lower, upper, 300)
...
for(deg in 1:19){
	...
	cm <- center_of_mass(proj, bound, 0, 18.5)
